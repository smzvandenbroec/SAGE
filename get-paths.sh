...
if [[ -f "$1" ]] && [[ "${1##*.}" == "dot" ]]; then
    ...
    sed 's/\]$/]KIRIL/g'  |     # Put a special "identifier" at the end of the line which represents an edge (excluding the edges from the objective variants to the objective node)
        ...
        sed 's/^\("[A-Zo. ]\+\)$/\1SENNE/' |    # Put some identifier
        sed 's/^\(.*\) \?-> \?\(.*\)$/\1 -> \2SENNE/' |     # Put some identifier
        ...
        awk -F '\t' '{ print $5 }' |    # Leave only nodes
        tr -d ',' |             # Delete unnecessary commas
        tr -s ' ' |             # Remove duplicate whitespaces
        sed 's/SENNE/\n/g' |    # Replace the identifier with a newline (to restore new line characters in the node names)
        sed 's/^ //' |          # Remove redundant leading whitespaces
